...
singleSampleSeg<- function(mSetsAnno, ArrayType, thresh, colour.amplification, colour.loss){
  ...
  #redefine segmean as gain (1), loss (-1), or neutral (0)
  segmentation_data$segmean_OLD <- segmentation_data$segmean
  segmentation_data$segmean <- ifelse(segmentation_data$segmean_OLD > (segmentation_data$noise * CNV_dynamic_cutoff),   "1", ifelse(segmentation_data$segmean_OLD < -(segmentation_data$noise * CNV_dynamic_cutoff), "-1", "0"))
...
singleSampleSeg2<- function(mSetsAnno, ArrayType, thresh, colour.amplification, colour.loss){
  ...
  x <- CNV.segment(CNV.bin(CNV.fit(query = mSetsAnno$target_mset_loaded, ref = mSetsAnno$control_mset_loaded, mSetsAnno$anno_targets)))
...
cum.CNV <- function(datafile, ArrayType = "overlap", segmentationMode = "all", thresh = 0.2, colour.amplification = "red3", colour.loss = "blue4", detail.regions = NULL){
  ...
  if (ArrayType == "EPIC") {
    rgset_EPIC <- minfi::read.metharray.exp(targets = target_files, force = TRUE)
...
cum.CNV(target_files, segmentationMode = "single", conumee.version = 1)
