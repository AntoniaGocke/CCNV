...
suppressPackageStartupMessages(library(conumee2.0))
...
if (ArrayType == "overlap") {
  ...
  combined <- combineArrays(rgset_EPIC, rgset_450k, outType = "IlluminaHumanMethylation450k")
  
  target_rgset <- combined
}
...
updatempcf <- function(data, gamma=5, assembly="hg19"){
...
#load and bin each sample in conumee 2.0
x <- CNV.bin(CNV.fit(query = target_mset_loaded, ref = control_mset_loaded, anno_targets))
...
test <- as.data.frame(x@bin$ratio)

target_ratios <- cbind(x@anno@bins@ranges@NAMES, x@anno@bins@ranges@start, test)

names(target_ratios) <- c("Chrom", "Median.bp", names(target_mset_loaded@intensity))
...
#start.time <- Sys.time()
seg_mpcf <- updatempcf(target_ratios, gamma = 5, assembly = "hg19")
#end.time <- Sys.time()
#time.taken <- end.time - start.time
#time.taken
...
bin_data_all <- as.data.frame(cbind(as.numeric(anno_targets@bins@ranges@start), as.numeric(rowMeans(bin_data_samples)), target_ratios$Chrom))
colnames(bin_data_all)<- c("bin_start", "bin_value", "names")

#genome data
chr_name <- anno_targets@genome$chr
chr_size <- anno_targets@genome$size
genome_chr <- cumsum(as.numeric(chr_size))
chr_centr <- anno_targets@genome$pq
...
#detail data
gene_regions <- as.data.frame(cbind(anno_EPIC@listData$UCSC_RefGene_Name, anno_EPIC@rownames, anno_EPIC@listData$chr, anno_EPIC@listData$pos))
...
#png(filename = "D:/Ependymoma/SegmentationTool/ETMR_Seq.png", width = 620, height = 480)
ggplot() +
  geom_point(final_bin_data, mapping = aes(x=V1, y=V2, colour=V2))+ 
  ylim(-2, 2) + 
  scale_color_gradient2(low = "dodgerblue3" ,  mid = "white",  high = "orangered3",  midpoint = 0, breaks = b, guide = "colourbar" , space = "Lab", name = "Intensity") + 
...
#png(filename = "D:/Ependymoma/SegmentationTool/ETMR_Freq.png", width = 620, height = 480)
ggplot() + 
  geom_rect(data = candidates,  aes(xmin = seg_start, xmax = seg_end, ymax = count, ymin = 0 , fill = candidates$col))  +
  geom_vline(xintercept = genome_chr, colour = "grey") +
  geom_vline(xintercept = genome_centr, linetype="dotted", colour = "grey") +
  ...
  scale_fill_manual(values = c("amplification" = "orangered3","deletion" = "dodgerblue3"), guide="none") +
  coord_cartesian(xlim = c(50001, 2881033286), expand = FALSE) +
  guides(x = guide_axis(n.dodge = 2))
```
...
# find overlapping probes between EPIC arrays and annotations
anno_targets@probes <- subsetByOverlaps(anno_targets@probes, granges(target_mset_mapped))

#start.time <- Sys.time()

#load and bin each sample in conumee
  foreach(i=1:ncol(target_mset_loaded@intensity)) %do%
  {
    if (i == 1) {
      x <- CNV.segment(CNV.bin(CNV.fit(query = target_mset_loaded[names(target_mset_loaded[i])], ref = control_mset_loaded, anno_targets)))
...
sample_no <- as.numeric(length(unique(segmentation_data$samplename)))
...
segmentation_data2 <- segmentation_data
#segmentation_data <- segmentation_data2
```
...
foreach(i=1:length(segmentation_data$chromosome)) %do%
{
 seg_start[i] <- segmentation_data$start[i] + addChr[segmentation_data$chromosome[i]]
  seg_end[i] <- segmentation_data$end[i] + addChr[segmentation_data$chromosome[i]] + 5000000
}
...
sample_no <- length(unique(segmentation_data$sample))

#png(filename = "D:/OverlayTest.png", width = 620, height = 480)
ggplot() + 
  ...
  coord_cartesian(xlim = c(50001, 2881033286), expand = FALSE) +
  guides(x = guide_axis(n.dodge = 2))+
  scale_fill_manual(values = alpha(c("amplification" = "red3","deletion" = "blue4"), 1/sample_no), guide="none")
```
...
```{r Test Freq plot}
 ...
 cnFreq_seg <- as.data.frame(cnFreq(segmentation_data, CN_low_cutoff = -thresh,  CN_high_cutoff = thresh, out = "data"))
  ...
  y_axis_break <-c(-1, -0.5, 0, 0.5, 1)
  y_axis_label <- c("100", "50", "0", "50", "100" )
 ...
 ggplot() + geom_rect(data = to_plot,  aes(xmin = start, xmax = end, ymax = count, ymin = 0 , fill = type))  +
    geom_vline(xintercept = genome_chr, colour = "grey") + 
    geom_vline(xintercept = genome_centr, linetype="dotted", colour = "grey") + 
    geom_hline(yintercept = 0, colour ="darkgrey") + 
    scale_x_continuous(name = "Chromosome", breaks = axis_break, labels = axis_label, limits = c(0, 2881033286)) +
    ...
    scale_fill_discrete(guide="none") + 
    scale_fill_manual(values = c("gain" = "red","loss" = "blue"), guide="none") + 
    coord_cartesian(xlim = c(50001, 2881033286), expand = FALSE) + 
    guides(x = guide_axis(n.dodge = 2))
```
...
chromosome_structure <- as.data.frame(anno_targets@genome)
chromosome_structure$cumul_size <- chromosome_structure$size
chromosome_structure$cumul_size <- c(0, head(chromosome_structure$cumul_size, -1))
chromosome_structure$cumul_size <- cumsum(as.numeric(chromosome_structure$cumul_size))

lookup_additional_size2 <- chromosome_structure[, c("chr", "cumul_size")]

cnFreq_seg$cumul_size <- lookup(cnFreq_seg$data.chromosome, lookup_additional_size)
cnFreq_seg$start.cumul <- cnFreq_seg$data.start + cnFreq_seg$cumul_size
cnFreq_seg$end.cumul <- cnFreq_seg$data.end + cnFreq_seg$cumul_size
cnFreq_seg$data.lossProportion <- cnFreq_seg$data.lossProportion*-1
...
gains_filtered_for_plot <- as.data.frame(cbind(gains$start.cumul, gains$end.cumul, gains$data.gainProportion, gains$type))
loss_filtered_for_plot <- as.data.frame(cbind(loss$start.cumul, loss$end.cumul, loss$data.lossProportion, loss$type))
...
to_plot$start <- as.numeric(to_plot$start)
to_plot$end <- as.numeric(to_plot$end)
to_plot$end <- to_plot$end + 10000000
to_plot$count <- as.numeric(to_plot$count)

axis_break <- genome_centr
axis_label <- c(1:22)

#pdf("E:/DMG/240214_DMG_Clust1_0-6_Capper_Control.pdf", height = 8, width = 14)
ggplot() + geom_rect(data = to_plot,  aes(xmin = start, xmax = end, ymax = count, ymin = 0 , fill = type))  +
  geom_vline(xintercept = genome_chr, colour = "grey") + 
  geom_vline(xintercept = genome_centr, linetype="dotted", colour = "grey") + 
  geom_hline(yintercept = 0, colour ="darkgrey") + 
  ...
  scale_fill_discrete(guide="none") + 
  scale_fill_manual(values = c("gain" = "red3","loss" = "blue4"), guide="none") + 
  coord_cartesian(xlim = c(50001, 2881033286), expand = FALSE) + 
  guides(x = guide_axis(n.dodge = 2))
